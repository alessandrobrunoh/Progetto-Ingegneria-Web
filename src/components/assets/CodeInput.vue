  ...
  setup(props) {
    ...
    const joinRoom = async () => {
      try {
        ...
        const response = await axios.get(`http://192.168.1.57:8000/api/room/${roomCode.value}`, {
          headers: {
        ...
        if (response.data.length > 0) {
          ...
          await axios.post(`http://192.168.1.57:8000/api/room/${roomCode.value}/add-player`, {}, {
            headers: {
              'Authorization': localStorage.getItem('token') // Assuming the token is stored in localStorage
            }
          });
